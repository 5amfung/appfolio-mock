...
TanStack Start's ISR uses standard HTTP cache headers — no vendor lock-in. Any CDN that respects `Cache-Control` works.

### Route-Level ISR

Add a `headers` function to a route to control CDN caching:
...
- `stale-while-revalidate=86400` — serve stale for up to 24 hours while regenerating.

### Server Function ISR

Cache API responses independently at the server function level:
...
export const getPost = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    return db.findPost(data.id)
  })
```

## SPA Mode

Disable server rendering entirely — suitable for internal tools/dashboards that don't need SEO:
...
tanstackStart({
  spa: {
    enabled: true,
    // Optional: pre-build the HTML shell.
    prerender: {
...
When `spa.enabled` is true, routes are not SSR-rendered. Server functions still work normally when deployed — only the page HTML is served statically.
